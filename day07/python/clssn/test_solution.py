...
@pytest.mark.parametrize('code, maxsig, phase_set', [
...
def test_brute1(code, maxsig, phase_set):
    p, m = solution.brute(code, feedback=False)
    assert p == phase_set
    assert m == maxsig
...
def test_brute2(code, maxsig, phase_set):
    p, m = solution.brute(code, feedback=True)
    assert p == phase_set
    assert m == maxsig
...
def test_amplify_fb(code, maxsig, phase_set):
    r = solution.amplify_fb(phase_set, code, inp=0)
    assert(r == maxsig)